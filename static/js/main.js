...
function domainvt(url, analysis_id) {
  ...
  if (url !== "" && url !== " ") {
    fetch(`/api/?domain=${url}&analysis_id=${analysis_id}`, {
      ...
      .catch((err) => {
        swal("Error!", "The api call failed! Is the analyzer offline?", "error");
        loading_div.style.display = "none";
      });
  } else {
  ...
  if (report_dir === "" || report_dir === " ") {
    ...
    fetch(apiurl, {
      ...
      .catch((err) => {
        swal(
          "Error!",
          "Something went wrong with the api call! Is the analyzer offline?",
          "warning"
        );
        loading_div.style.display = "none";
      });
...
function geoip(ip) {
  ...
  if (ip !== "" && ip !== " ") {
    fetch(`/api/?ip=${ip}`, {
      ...
      .catch((err) => {
        swal("Error!", "API Call failed! Is the analyzer offline?", "error");
        loading_div.style.display = "none";
      });
  } else {
...
function update() {
  swal(
    "Update Analyzer",
    'Use the command "python3 extanalyser.py --update" to check for updates!',
    "info"
  );
}
